...
<div class="col-span-10 h-[100px] p-3"> <!-- Quita la clase bg-gray-200 y aplica una altura fija -->
    <br>
    <h1 class="text-2xl font-bold mb-4 text-center">REPORTE DE DEVOLUCIONES PENDIENTES</h1>
    </div>
</div>


<form action="{{ route('ejecutarExpedienteAtrasosSU') }}" method="POST" class="mb-4">
    @csrf
    <input type="hidden" name="elementos" value="{{ json_encode($elementos) }}">
        ...
        <select id="filtro" name="filtro" class="w-full p-2 border border-gray-300 rounded">
            ...
            <option value="3">Pendientes y Atrasos</option>
        </select>
        </div>
        ...
        <div class="flex justify-center gap-4">
            ...
            <a href="{{ route('exportarExpedientesR4', ['elementos' => $elementos]) }}" class="mt-5 bg-blue-500 hover:bg-blue-600 text-white font-medium rounded-lg text-sm py-2 px-4">Exportar</a>
    @else
