...
<div class="col-span-2 flex-grow flex-shrink p-6">
    ...
    <div class="col-span-8 h-[100px] p-3"> <!-- Quita la clase bg-gray-200 y aplica una altura fija -->
        <form class="rounded mb-4 bg-transparent" action="{{route('usuario.search')}}" method="POST"> <!-- Agrega una clase personalizada para quitar el fondo de color -->
            @csrf
            ...
            <div class="relative">
                ...
                <input type="search" id="default-search" class="block w-full p-6 pl-10 text-bg text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Número o nombre de usuario" autofocus="autofocus" required type="text" name="usuario" id="usuario">
                <button id="usuario" type="submit" class="text-white absolute right-2.5 bottom-2.5 bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-8 py-4 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" type="submit">Buscar</button>
            </div>
        </form>
    </div>
...
<h1 class="text-2xl font-bold mb-4 text-center">USUARIOS REGISTRADOS</h1>
    ...
    <tr>
        <th scope="col" class="px-6 py-3" style="width: 10%;">Número</th>
        <th scope="col" class="px-6 py-3" style="width: 35%;">Nombre</th>
        <th scope="col" class="px-6 py-3" style="width: 20%;">Apellidos</th>
        <th scope="col" class="px-6 py-3" style="width: 25%;">Tipo de usuario</th>
    ...
    @if ($elemento->rol == 1)
        Básico
    @elseif ($elemento->rol == 2)
        Administrador
    @elseif ($elemento->rol == 3)
        Super Administrador
    @else
        Otro Rol
    @endif
                            ...
                            <td class="px-6 py-4 text-left">
                                <a href="{{route('detallesUsuario',$elemento->idUsuarioSistema)}}">Detalles</a>
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
