...
<script>
        ...
        if (successMessage) {
            ...
            // Espera 10 segundos y luego oculta el mensaje de info
            setTimeout(function () {
...
<div class="col-span-2 flex-grow flex-shrink p-6">
    <a href="{{ route('expedientesBasico', $usuario->idUsuarioSistema) }}">
    ...
    <div class="col-span-8 h-[100px] p-3"> <!-- Quita la clase bg-gray-200 y aplica una altura fija -->
        <form class="rounded mb-4 bg-transparent" action="{{route('clientesUsuario.search')}}" method="POST"> <!-- Agrega una clase personalizada para quitar el fondo de color -->
            @csrf
            <input type="hidden" name="id_usuario" value="{{$usuario->idUsuarioSistema}}">
            ...
            <div class="relative">
                ...
                <input type="search" id="default-search" class="block w-full p-6 pl-10 text-bg text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Buscar Expediente" autofocus="autofocus" required type="text" name="id_cliente" id="id_cliente">
                <button id="id_cliente" type="submit" class="text-white absolute right-2.5 bottom-2.5 bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-8 py-4 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" type="submit">Buscar</button>
            </div>
        </form>
    </div>

    <div class="col-span-2 flex-grow flex-shrink p-6">
    <a href="{{ route('cliente.nuevoBasico', $usuario->idUsuarioSistema) }}">
...
<h1 class="text-2xl font-bold mb-4 text-center">EXPEDIENTES REGISTRADOS</h1>
    ...
    <tr>
        <th scope="col" class="px-6 py-3" style="width: 8%;">Número de Expediente</th>
        <th scope="col" class="px-6 py-3" style="width: 50%;">Nombre de Expediente</th>
        <th scope="col" class="px-6 py-3" style="width: 30%;">ID Cliente Giro</th>
        <th scope="col" class="px-6 py-3" style="width: 12%;"></th>
    </tr>
</thead>
    </table>
            </div>
    ...
    @foreach($elementos as $elemento)
        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
            @if(!empty($elemento->id_consecutivo) && !empty($elemento->id_clienteGiro) && $elemento->id_consecutivo !== 0)
                <td class="px-6 py-4 whitespace-nowrap dark:text-white">
                    {{ $elemento->id_consecutivo }}
                </td>
                <td class="px-6 py-4 whitespace-nowrap dark:text-white">
                    {{ $elemento->nombre }}
                </td>
                ...
                <td class="px-6 py-4 text-right">
                    <a href="{{ route('cliente.asignados', $elemento->id_consecutivo) }}">Asignados</a>
                    <a href="{{ route('clienteNuevoExpediente', $elemento->id_consecutivo) }}">Asignar</a>
                </td>
            @else
            <td class="px-6 py-4 whitespace-nowrap dark:text-white">
            @if(!empty($elemento->id_consecutivo) && $elemento->id_consecutivo !== 0)
                {{$elemento->id_consecutivo}}
                @else
                NA
                @endif                
                ...
                <td class="px-6 py-4 whitespace-nowrap dark:text-white">
                @if(!empty($elemento->id_clienteGiro))
                {{$elemento->id_clienteGiro}}
                @else
                NA
                @endif
                ...
                <td class="px-6 py-4 text-right">
                <a href="{{ route('cliente.edit', $elemento->id_cliente) }}">Asignar Número Expediente</a>
                </td>
            @endif
        </tr>
    @endforeach
